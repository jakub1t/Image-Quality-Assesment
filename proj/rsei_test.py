...
def enforce_label_connectivity(labels, K=5):
    H, W = labels.shape
    SUPSZ = (H * W) / K
    ...
    for y in range(H):
        for x in range(W):
            ...
            for k in range(4):
                xn, yn = x + dx[k], y + dy[k]
                if 0 <= xn < W and 0 <= yn < H and nlabels[yn, xn] > 0:
                    adjlabel = nlabels[yn, xn]
            ...
            while c < count:
                for k in range(4):
                    xn = xvec[c] + dx[k]
                    yn = yvec[c] + dy[k]
                    if (0 <= xn < W and 0 <= yn < H and
                        nlabels[yn, xn] < 0 and labels[yn, xn] == labels[y, x]):
                        nlabels[yn, xn] = label
                        xvec[count] = xn
                        yvec[count] = yn
                        count += 1
...
def calculate_rsei(reference_image, deformed_image):
    ...
    nlabels = enforce_label_connectivity(klabels, K)
